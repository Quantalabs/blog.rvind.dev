...
The method in **(1.1)**, while not intuitive, can be used to explain derivations of volume, but introduces complexities very quickly as we move to more advanced problems such as **(1.2a)** below.

> **Example 1.2a** Determine the volume of a solid whose base is the region contained by $y=(9-x^2)^{1/2}$,$x=1$, $x=3$, and $y=0$, and has cross-sections perpendicular to the $x$-axis that are equilateral triangles.
...
Despite already having more complexities, **(1.2a)** can get harder. For example, one could change the boundary from $y=0$ to $y=10x$, and rotate about the $y$-axis, resulting in an even less intuitive problem. Although thinking about "area under the curve" works, constantly translating between 2-D and 3-D is a barrier to a deeper understanding.
...
> **Example 1.2b** Determine the volume of a solid whose base is the region contained by $y=(9-x^2)^{1/2}$,$x=1$, $x=3$, and $y=0$, and has cross-sections perpendicular to the $x$-axis that are equilateral triangles.
...
We are no longer trying to find the area under the curve of a function. Instead, we are thinking about adding up all the values of the function at each point, hence why we can use integrals in so many different contexts, like the average value of a function or the arc length of a curve.

## Approaches to Problem Solving

The sum definition gives us a powerful tool for problem solving. Let's condense the methodology we applied in **(1.2b)** into a set of steps we may follow when approaching a problem relating to integration:

1. Find what you are adding up, e.g a triangle, set of $y$-values, etc.
2. Find the range you are adding over, e.g. a range of $x$-values, $y$-values, or even a time interval. Keep in mind which variable you are changing, the variable will let you choose to integrate with respect to $dx$, $dt$, etc.
3. Take your function from step 1 and multiply it by the differential (e.g. $dx$, $dy$, etc.) from step 2, which yields the area of a rectangle, which is the change in your antiderivative.
4. Integrate the function and differential over the range from step 2, which bceomes your final answer!

Let's attempt to use the above methodology with a new type of problem you may have encountered before.
...
4. Now, we can integrate the expression from 0 to $\pi$:
...
I think it is important to note that the manipulation of $dx$ and $dy$ that we performed in step 3 is frowned upon by mathematicians because there exist contexts in which we cannot treat $dx$ and $dy$ as variables. However, for the purposes of introductory calculus, the method provides much better intuition, and we don't work with these contexts until later on in our mathematical education. Therefore, it is best to avoid using such manipulation on a test or formal setting, but the method remains a great way to build intuition.
